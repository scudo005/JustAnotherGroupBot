...
try:
    with open("chatmembers.json", "r") as chatMembersFileHandler:
        _chat_member_username_id = json.load(chatMembersFileHandler)
except FileNotFoundError:
    print("La lista degli utenti è vuota.")
    # sys.exit()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await context.bot.send_message(
        chat_id=update.effective_chat.id, text="Bot pronto. Richiede permessi di admin."
    )


async def ban_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    print(f"ban {context.args[0]}")
    __member_to_ban_username = context.args[0]  # obtain only the raw username
    # usernames must be at least 4 characters long.
    if (len(__member_to_ban_username) < 4) and "@" in __member_to_ban_username:
        await context.bot.send_message(
            chat_id=update.effective_chat.id, text="Username non valido."
        )
    else:  # here we have a valid username
        __banned = get_id_from_username(__member_to_ban_username)
        if __banned == 0:
            await context.bot.send_message(
                chat_id=update.effective_chat.id, text="Username non trovato."
            )
        else:
            try:
                successful_ban = await context.bot.ban_chat_member(
                    chat_id=update.effective_chat.id, user_id=__banned
                )
                # this should never happen according to the docs.
                if successful_ban is False:
                    await context.bot.send_message(
                        chat_id=1091498287,
                        text=f"Ban di {__member_to_ban_username} fallito, controlla i log.",
                    )
                else:
                    await context.bot.send_message(
                        chat_id=update.effective_chat.id,
                        text=f"{__member_to_ban_username} bannato.",
                    )
                ...
                await context.bot.send_message(
                    chat_id=update.effective_chat.id,
                    text=f"Canale {__member_to_ban_username} bannato.",
                )
...
async def unban_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(__member_to_unban_username) <= 4:
        ...
        if __unbanned == 0:
            ...
            try:
                successful_unban = await context.bot.unban_chat_member(
                    chat_id=update.effective_chat.id,
                    user_id=__unbanned,
                    only_if_banned=True,
                )
                # this should never happen according to the docs.
                if successful_unban is False:
                    await context.bot.send_message(
                        chat_id=1091498287,
                        text=f"Unban di {__member_to_unban_username} fallito, controlla i log.",
                    )
                else:
                    await context.bot.send_message(
                        chat_id=update.effective_chat.id,
                        text=f"{__member_to_unban_username} sbannato.",
                    )
            except:  # we are trying to unban a channel
                await context.bot.unban_chat_sender_chat(
                    chat_id=update.effective_chat.id, user_id=__unbanned
                )
                await context.bot.send_message(
                    chat_id=update.effective_chat.id,
                    text=f"Canale {__member_to_ban_username} sbannato.",
                )
...
async def kick_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(__member_to_kick_username) <= 4:
        ...
        if __kicked == 0:
            ...
            try:
                successful_ban = await context.bot.ban_chat_member(
                    chat_id=update.effective_chat.id, user_id=__kicked
                )
                successful_unban = await context.bot.unban_chat_member(
                    chat_id=update.effective_chat.id,
                    user_id=__kicked,
                ...
                # this should never happen according to the docs.
                if successful_unban is False or successful_ban is False:
                    await context.bot.send_message(
                        chat_id=update.effective_chat.id,
                        text=f"Kick di {__member_to_kick_username} fallito, controlla i log.",
                    )
                else:
                    await context.bot.send_message(
                        chat_id=update.effective_chat.id,
                        text=f"{__member_to_kick_username} kickato.",
                    )
            except:  # we are trying to kick a channel
                await context.bot.send_message(
                    chat_id=update.effective_chat.id,
                    text="Non puoi kickare un canale, leggi i docs idiota",
                )
